    ...
    <>
      ...
      <Card title="💾 Download Reports & Files" style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={() => exportPlatePDF(results, material, reuseMin)} style={EXP}>📄 Procurement PDF</button>
          <button onClick={() => exportPlateExcel(results, material, reuseMin)} style={EXP}>📊 Procurement Excel</button>
          {results.totals.offcutCount > 0 && <button onClick={() => exportOffcutPDF(results, material, reuseMin)} style={{ ...EXP, background: "linear-gradient(135deg,#10b981,#059669)", color: "#04140d" }}>♻ Leftover PDF</button>}
          {results.totals.offcutCount > 0 && <button onClick={() => exportOffcutExcel(results, material, reuseMin)} style={{ ...EXP, background: "linear-gradient(135deg,#10b981,#059669)", color: "#04140d" }}>♻ Leftover Excel</button>}
        </div>
      </Card>
  ...
  return (
    <>
      ...
      <Card title="💾 Download Reports & Files">
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={() => exportSectionPDF(results, kerf, { totalStock, wastePct, wasteMm, wasteKg })} style={EXP}>📄 Procurement PDF</button>
          <button onClick={() => exportSectionExcel(results)} style={EXP}>📊 Procurement Excel</button>
          {hasLeftovers && <button onClick={() => exportSectionLeftoverPDF(results)} style={{ ...EXP, background: "linear-gradient(135deg,#10b981,#059669)", color: "#04140d" }}>♻ Leftover PDF</button>}
          {hasLeftovers && <button onClick={() => exportSectionLeftoverExcel(results)} style={{ ...EXP, background: "linear-gradient(135deg,#10b981,#059669)", color: "#04140d" }}>♻ Leftover Excel</button>}
        </div>
      </Card>
    </>
  );
...
function exportSectionExcel(results) {
  ...
  // ── 3) CUTTING PLAN (bar by bar) ──
  const plan = [["CUTTING PLAN — bar by bar"], [], ["Profile", "Grade", "Bar #", "Available Length (mm)", "Cuts (mm)", "Pieces", "Leftover (mm)", "Leftover Reusable?", "Utilization %"]];
  results.groups.forEach(g => g.bins.forEach((b, i) => plan.push([g.profile, g.grade || "", i + 1, b.stockLength, b.cuts.map(c => Math.round(c.length)).join(" + "), b.cuts.length, Math.round(b.remaining), b.remaining >= 1000 ? "YES (≥1 m)" : "no", Math.round(((b.stockLength - b.remaining) / b.stockLength) * 1000) / 10])));
  wsAdd("Cutting Plan", plan, [{ wch: 18 }, { wch: 12 }, { wch: 7 }, { wch: 20 }, { wch: 40 }, { wch: 8 }, { wch: 14 }, { wch: 18 }, { wch: 14 }]);
