...
function splitPart(W, L, uw, uh, allowRotation) {
  // Tile a part too big for the sheet into a grid of welded sub-pieces. Pick the
  // orientation that needs the FEWEST pieces (fewest welds).
  const grid = (a, b) => {
    const cols = Math.max(1, Math.ceil(a / uw)), rows = Math.max(1, Math.ceil(b / uh));
    const list = [];
    for (let c = 0; c < cols; c++) for (let r = 0; r < rows; r++) list.push({ w: Math.round(Math.min(uw, a - c * uw)), h: Math.round(Math.min(uh, b - r * uh)) });
    return list;
  };
  const g1 = grid(W, L);
  if (allowRotation) { const g2 = grid(L, W); if (g2.length < g1.length) return g2; }
  return g1;
}

function nestPlates(sheetW, sheetH, parts, kerf, margin, allowRotation, reuseMin) {
  const sheets = [];
  ...
  parts.forEach(p => {
    for (let q = 0; q < p.qty; q++) {
      if (fitsRaw(p.width, p.length)) { queue.push({ w: p.width, h: p.length, id: p.id, label: p.label }); }
        ...
        const subs = splitPart(p.width, p.length, usableW, usableH, allowRotation);
  ...
  while (queue.length > 0 && guard < 20000) {
    ...
    if (placements.length === 0 && remaining.length > 0) { const p = remaining.shift(); sheets.push({ placements: [{ x: m, y: m, w: Math.min(p.w, usableW), h: Math.min(p.h, usableH), rotated: false, id: p.id, label: p.label, spliced: p.spliced }], offcuts: [] }); }
      ...
      const offcuts = [];
      rows.forEach(r => { const stripW = usableW - r.usedW; if (stripW >= reuseMin && r.rowH >= reuseMin) offcuts.push({ w: Math.round(stripW), h: Math.round(r.rowH) }); });
      const lastBottom = rows.length ? rows[rows.length - 1].y + rows[rows.length - 1].rowH : m;
      const bottomH = (m + usableH) - lastBottom;
      if (bottomH >= reuseMin && usableW >= reuseMin) offcuts.push({ w: Math.round(usableW), h: Math.round(bottomH) });
      sheets.push({ placements, offcuts });
    }
    queue = remaining;
  }
...
function PlateCanvas({ sheets, sheetW, sheetH, colorMap, thickness }) {
  ...
  useEffect(() => {
    ...
    sheet.forEach((p, i) => { const px = p.x * SCALE, py = p.y * SCALE, pw = p.w * SCALE, ph = p.h * SCALE; const color = colorMap[p.id] || PART_COLORS[i % PART_COLORS.length]; ctx.fillStyle = color + "55"; ctx.fillRect(px, py, pw, ph); ctx.strokeStyle = color; ctx.lineWidth = 1.5; ctx.strokeRect(px, py, pw, ph); const fs = Math.max(8, Math.min(11, pw / 6, ph / 3)); ctx.fillStyle = "rgba(255,255,255,.92)"; ctx.font = `${fs}px 'Space Mono', monospace`; ctx.textAlign = "center"; ctx.textBaseline = "middle"; if (pw > 22 && ph > 13) { ctx.fillText(p.rotated ? `${p.label}↺` : p.label, px + pw / 2, py + ph / 2 - (ph > 26 ? 6 : 0)); if (ph > 26) { ctx.font = `${Math.max(7, fs - 1)}px 'Space Mono', monospace`; ctx.fillStyle = "rgba(230,210,170,.8)"; ctx.fillText(`${Math.round(p.w)}×${Math.round(p.h)}`, px + pw / 2, py + ph / 2 + 7); } } });
  ...
  return (
    <div style={{ fontFamily: "'Space Mono', monospace" }}>
      ...
      {offcuts.length > 0 && <div style={{ marginTop: 8, padding: "8px 12px", background: "rgba(16,185,129,.08)", border: "1px solid rgba(16,185,129,.3)", borderRadius: 6, fontSize: 11, color: "#6ee7b7" }}>♻ Reusable offcut{offcuts.length > 1 ? "s" : ""}: {offcuts.map(o => `${o.w}×${o.h}mm`).join(" · ")}</div>}
...
function PlatesModule({ onBack }) {
  ...
  const [kerf, setKerf] = useState(3); const [margin, setMargin] = useState(10); const [reuseMin, setReuseMin] = useState(300); const [allowRotation, setAllowRotation] = useState(true);
  ...
  const optimize = () => {
    ...
    setTimeout(() => {
      ...
      const groups = Object.keys(byThk).map(Number).sort((a, b) => b - a).map(thk => {
        ...
        const gsheets = nestPlates(sw, sh, gp, kerf, margin, allowRotation, reuseMin);
        ...
        const offcuts = []; gsheets.forEach((st, si) => (st.offcuts || []).forEach(o => offcuts.push({ sheet: si + 1, w: o.w, h: o.h, weight: (o.w * o.h * thk / 1e9) * DENSITY })));
    ...
    <>
      ...
      {results.totals.offcutCount > 0 && (
        <Card title="♻ Reusable Offcut Register" style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 12, color: "#6ee7b7", marginBottom: 12, background: "rgba(16,185,129,.08)", border: "1px solid rgba(16,185,129,.25)", borderRadius: 6, padding: "8px 12px" }}>{results.totals.offcutCount} reusable offcut{results.totals.offcutCount > 1 ? "s" : ""} ≈ {fmtTon(results.totals.offcutWeight)} t to keep for future jobs (≥ {reuseMin} mm/side).</div>
          <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'Space Mono', monospace" }}><thead><tr style={{ background: "rgba(16,185,129,.12)", borderBottom: "1px solid rgba(16,185,129,.3)" }}>{["THICKNESS", "FROM SHEET", "OFFCUT SIZE", "WEIGHT", "STATUS"].map((h, i) => <th key={h} style={{ padding: "10px 12px", textAlign: i > 2 ? "right" : "left", color: "#6ee7b7", fontSize: 11, letterSpacing: 1, fontWeight: 700 }}>{h}</th>)}</tr></thead>
            <tbody>{results.groups.flatMap(g => g.offcuts.map((o, oi) => <tr key={`${g.thickness}-${oi}`} style={{ borderBottom: "1px solid #1a2230" }}><td style={{ padding: "9px 12px", color: "#cbd5e1" }}>{g.thickness} mm</td><td style={{ padding: "9px 12px", color: "#94a3b8" }}>Sheet {o.sheet}</td><td style={{ padding: "9px 12px", color: "#cbd5e1" }}>{o.w} × {o.h} mm</td><td style={{ padding: "9px 12px", textAlign: "right", color: "#94a3b8" }}>{o.weight >= 1 ? `${o.weight.toFixed(1)} kg` : `${(o.weight * 1000).toFixed(0)} g`}</td><td style={{ padding: "9px 12px", textAlign: "right", color: "#10B981", fontWeight: 700 }}>✓ keep</td></tr>))}</tbody>
          </table></div>
...
function SectionsModule({ onBack }) {
  ...
  const SECTION_RX = /\b(?:IPE|IPN|HEA|HEB|HEM|HE|UBP|UB|UC|HW|HM|HN|RHS|SHS|CHS|UPN|UPE|PFC|PIPE)\s?\d+(?:\s?[x×*]\s?\d+(?:\.\d+)?)*\b|\b\d+x\d+x\d+\s?(?:UB|UC|UBP|PFC)\b|\b(?:HW|HM|HN|C|L|I)\s?\d+x\d+(?:x\d+)?\b|\bW\d+x\d+\b/i;
  const norm = s => String(s == null ? "" : s).replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();
...
function exportOffcutExcel(results, material, reuseMin) {
  ...
  const rows = [["Reusable Offcut Register"], ["Material", material], [`Minimum size kept (each side)`, `${reuseMin} mm`], [],
    ["#", "Thickness (mm)", "From Sheet", "Width (mm)", "Length (mm)", "Weight (kg)", "Status"]];
  let n = 0; results.groups.forEach(g => g.offcuts.forEach(o => { n++; rows.push([n, g.thickness, `Sheet ${o.sheet}`, o.w, o.h, o.weight.toFixed(2), "keep for future jobs"]); }));
...
function exportOffcutPDF(results, material, reuseMin) {
  const rowsHtml = results.groups.flatMap(g => g.offcuts.map((o, k) => `<tr><td>${g.thickness} mm</td><td>Sheet ${o.sheet}</td><td>${o.w} x ${o.h} mm</td><td>${o.weight >= 1 ? o.weight.toFixed(1) + " kg" : (o.weight * 1000).toFixed(0) + " g"}</td><td>keep for future jobs</td></tr>`)).join("");
  ...
  <div class="banner">${results.totals.offcutCount} reusable offcut${results.totals.offcutCount !== 1 ? "s" : ""} &asymp; <b>${fmtTon(results.totals.offcutWeight)} t</b> to keep for future jobs.</div>
  <table><tr><th>Thickness</th><th>From Sheet</th><th>Offcut Size</th><th>Weight</th><th>Status</th></tr>
  ${rowsHtml || `<tr><td colspan="5">No reusable offcuts above the minimum size.</td></tr>`}
  <tr class="total"><td colspan="3">TOTAL — ${results.totals.offcutCount} pieces</td><td>${fmtTon(results.totals.offcutWeight)} t</td><td></td></tr></table>
...
function exportPlatePDF(results, material, reuseMin) {
  const offRows = results.groups.flatMap(g => g.offcuts.map(o => `<tr><td>${g.thickness} mm</td><td>Sheet ${o.sheet}</td><td>${o.w} x ${o.h} mm</td><td>${o.weight.toFixed(1)} kg</td><td>keep for future jobs</td></tr>`)).join("");
