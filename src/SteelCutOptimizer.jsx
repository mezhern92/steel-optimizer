...
function LPointBuilder({ points, onChange }) {
  ...
  const corners = ["①", "②", "③", "④", "⑤", "⑥"];
  const hints = ["start corner — leave at 0, 0", "→ go along the bottom edge", "↑ up the side", "← into the notch", "↑ up to the top", "← back toward start"];
  return (
    <div style={{ display: "flex", gap: 18, flexWrap: "wrap", alignItems: "flex-start" }}>
      <div style={{ flex: "1 1 220px", minWidth: 210 }}>
        <div style={{ fontSize: 12, color: "#6ee7b7", lineHeight: 1.6, marginBottom: 12, background: "rgba(16,185,129,.08)", border: "1px solid rgba(16,185,129,.25)", borderRadius: 8, padding: "9px 12px" }}>
          Stand at one corner of your leftover and call it <b>0, 0</b>. Walk the outline and type each corner you reach — the drawing builds as you go.
        </div>
        <table style={{ borderCollapse: "separate", borderSpacing: "0 7px" }}>
          <thead><tr><th></th><th style={{ fontSize: 9, color: "#64748b", letterSpacing: 1, textAlign: "left", paddingLeft: 6 }}>X · width →</th><th style={{ fontSize: 9, color: "#64748b", letterSpacing: 1, textAlign: "left", paddingLeft: 6 }}>Y · length ↑</th></tr></thead>
          <tbody>
            {pts.map((p, i) => (
              <tr key={i}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => { const d = defaultLPoints(); onChange(d, analyzeL(d)); }} style={{ ...CI, cursor: "pointer", borderColor: "#334155", padding: "6px 12px" }}>↺ Reset shape</button>
        </div>
      </div>
      <div style={{ flex: "0 0 auto" }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: 360, background: "linear-gradient(160deg, rgba(13,18,24,.92), rgba(10,14,20,.6))", borderRadius: 12, border: "1px solid rgba(148,163,184,.16)" }}>
          ...
          <text x={W - 18} y={oy + 16} fill="#fbbf24" fontSize="9" textAnchor="end" fontFamily="'Space Mono', monospace">X · width</text>
          <text x={ox - 8} y={20} fill="#fbbf24" fontSize="9" textAnchor="start" fontFamily="'Space Mono', monospace" transform={`rotate(-90 ${ox - 8} 20)`}>Y · length</text>
          {/* usable rectangle */}
          ...
          {uRect && uRect.w > 0 && uRect.h > 0 && (
            <text x={SX(uRect.x + uRect.w / 2)} y={SY(uRect.y + uRect.h / 2)} fill="#6ee7b7" fontSize="10" fontWeight="700" textAnchor="middle" fontFamily="'Space Mono', monospace">USABLE</text>
          )}
        </svg>
        <div style={{ marginTop: 8, textAlign: "center", fontSize: 11, color: info.valid ? "#6ee7b7" : "#fbbf24", fontFamily: "'Space Mono', monospace" }}>
          Overall <b>{Math.round(info.A)} × {Math.round(info.B)}</b> · usable <b style={{ color: "#6ee7b7" }}>{Math.round(info.usable.w)} × {Math.round(info.usable.h)}</b>
          {!info.valid && <div style={{ color: "#fbbf24", marginTop: 3 }}>tip: an L has 6 corners forming a clean step</div>}
        </div>
      </div>
    </div>
  );
...
function LeftoverImporter({ expects, onImported }) {
  ...
  const pick = async file => {
    ...
    try {
      ...
      if (expects && res.type !== expects) { setMsg(`⚠ That looks like a ${res.type} leftover file. Use it in the ${res.type === "plates" ? "Plates" : "Sections"} module.`); setBusy(false); return; }
      const n = res.items.reduce((s, it) => s + (it.qty || 1), 0);
      onImported(res.items);
      setMsg(`✓ Imported ${res.items.length} leftover type${res.items.length > 1 ? "s" : ""} (${n} piece${n > 1 ? "s" : ""}). They'll be cut from first.`);
    } catch (e) { setMsg("⚠ " + (e.message || "Could not read this file.")); }
    setBusy(false);
  };
  return (
    <div style={{ marginBottom: 12 }}>
      <button onClick={() => ref.current?.click()} disabled={busy} style={{ padding: "9px 18px", background: "rgba(59,130,246,.12)", border: "1px dashed #3b82f6", color: "#93c5fd", borderRadius: 8, cursor: busy ? "wait" : "pointer", fontSize: 13, fontWeight: 700, fontFamily: "'Space Mono', monospace" }}>
        {busy ? "⏳ Reading…" : "⤓ Reuse a previous Leftover file (PDF / Excel)"}
      </button>
      ...
      const groups = Object.keys(byThk).map(Number).sort((a, b) => b - a).map(thk => {
        ...
        myOffcuts.forEach(o => {
          ...
          for (let u = 0; u < units; u++) {
            ...
            if (!res.placedAny) break;            // nothing fits this piece → identical pieces won't help
            ...
            gp = res.leftover;
            if (!gp.length) break;                 // all parts placed
          }
        });
    ...
    <>
      ...
      {inputMode !== null && (
        <Card title="♻ Reuse leftover plates (optional)" style={{ marginTop: 20 }}>
          <div style={{ fontSize: 13, color: "#6ee7b7", marginBottom: 14, lineHeight: 1.6 }}>Got leftover plates from a past job — or from another site? Add them here (or import a previous Leftover file) and we'll cut from them <b>first</b>, so you only buy the <b>minimum</b> new sheets you still need.</div>
              ...
              <div style={{ flex: "1 1 340px", minWidth: 290 }}>
                ...
                {o.shape === "rect" ? (
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    ...
                    <div><Label>How many? (qty)</Label><input type="number" min="1" value={o.qty} onChange={e => upOffcut(i, "qty", e.target.value)} style={{ ...CI, width: 90, borderColor: "#10b98199" }} /></div>
                  </div>
                ) : (
                  <div>
                ...
                <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 8, background: "rgba(16,185,129,.1)", border: "1px solid rgba(16,185,129,.3)", fontSize: 12, color: "#6ee7b7" }}>✓ We'll nest parts into a <b>{Math.round(use.w)} × {Math.round(use.h)} mm</b> area{o.shape === "L" ? " (the biggest rectangle that fits the L)" : ""}{(o.qty || 1) > 1 ? <> × <b>{Math.round(o.qty)}</b> piece{o.qty > 1 ? "s" : ""}</> : ""}.</div>
              </div>
              {o.shape === "rect" && <div style={{ flex: "0 0 auto" }}><LShapeDiagram A={o.A} B={o.B} C={0} D={0} t={o.thickness} /></div>}
            </div>
          ); })}
          {offcutStock.length > 0 && <button onClick={addOffcut} style={{ marginTop: 4, padding: "9px 20px", background: "rgba(16,185,129,.12)", border: "1px dashed #10b981", color: "#6ee7b7", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>+ Add a leftover from another job / site</button>}
        </Card>
      )}
      {inputMode === "manual" && (
...
function SectionsModule({ onBack }) {
  ...
  const runOpt = list => {
    ...
    if (!valid.length) { setError("No valid cuts."); return; }
    const groups = {};
  ...
  return (
    <>
      ...
      {inputMode !== null && (
        <Card title="♻ Reuse leftover bars (optional)" style={{ marginTop: 20 }}>
          <div style={{ fontSize: 13, color: "#6ee7b7", marginBottom: 14, lineHeight: 1.6 }}>Got leftover bars from a past job — or from another site? Add them here (or import a previous Leftover file) and we'll cut from them <b>first</b>, so you only buy the <b>minimum</b> new bars you still need.</div>
          ...
          {leftovers.length === 0 && (
            <div style={{ padding: "20px 18px", textAlign: "center", border: "1px dashed rgba(16,185,129,.35)", borderRadius: 12, background: "rgba(16,185,129,.04)" }}>
              <div style={{ fontSize: 13, color: "#94a3b8", marginBottom: 12 }}>No leftover bars added yet.</div>
              <button onClick={addLeftover} style={{ padding: "10px 22px", background: "linear-gradient(135deg,#10b981,#059669)", border: "none", color: "#04140d", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>+ Add a leftover bar</button>
            </div>
          )}
          {leftovers.length > 0 && (
            <>
              <div style={{ overflowX: "visible" }}><table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 10px" }}><thead><tr>{["Profile", "Grade", "Leftover length (mm)", "How many? (qty)", ""].map((h, i) => <th key={h} style={{ textAlign: "left", padding: "0 8px", color: "#64748b", fontSize: 11, letterSpacing: 1, width: i === 0 ? "30%" : i === 1 ? "18%" : i === 4 ? 40 : "auto" }}>{h}</th>)}</tr></thead>
                <tbody>{leftovers.map(r => { const matches = loAuto === r.id ? searchSections(r.profile) : []; const sec = findSection(r.profile); return (
                  <tr key={r.id}>
                    <td style={{ position: "relative", padding: "0 8px" }}><input style={{ ...INL, borderColor: "#10b98155" }} placeholder="IPE 300" value={r.profile} onChange={e => { upLeftover(r.id, "profile", e.target.value); setLoAuto(r.id); }} onFocus={() => setLoAuto(r.id)} onBlur={() => setTimeout(() => setLoAuto(a => a === r.id ? null : a), 150)} />{sec && <div style={{ fontSize: 11, color: sec.computed ? "#60a5fa" : "#22c55e", fontFamily: "'Space Mono', monospace", marginTop: 5 }}>{sec.kgm} kg/m · {sec.type}</div>}{matches.length > 0 && <div style={{ position: "absolute", top: "100%", left: 8, right: 8, zIndex: 50, background: "#0f1318", border: "1px solid #2d3748", borderRadius: 6, maxHeight: 260, overflowY: "auto", boxShadow: "0 8px 24px rgba(0,0,0,.5)" }}>{matches.map(m => <div key={m.name} onMouseDown={() => { upLeftover(r.id, "profile", m.name); setLoAuto(null); }} style={{ padding: "11px 14px", cursor: "pointer", display: "flex", justifyContent: "space-between", borderBottom: "1px solid #1a2230", fontFamily: "'Space Mono', monospace", fontSize: 13 }}><span style={{ color: "#cbd5e1" }}>{m.name}</span><span style={{ color: "#f59e0b" }}>{m.kgm} kg/m</span></div>)}</div>}</td>
                    ...
                    <td style={{ padding: "0 8px" }}><input style={INL} type="number" placeholder="6000" value={r.length} onChange={e => upLeftover(r.id, "length", e.target.value)} /><div style={{ fontSize: 11, color: "#475569", fontFamily: "'Space Mono', monospace", marginTop: 5 }}>length of the bar you have left</div></td>
                    <td style={{ padding: "0 8px" }}><input style={{ ...INL, borderColor: "#10b98199" }} type="number" min="1" placeholder="1" value={r.qty} onChange={e => upLeftover(r.id, "qty", e.target.value)} /><div style={{ fontSize: 11, color: "#475569", fontFamily: "'Space Mono', monospace", marginTop: 5 }}>how many like this</div></td>
              ...
              <button onClick={addLeftover} style={{ marginTop: 4, padding: "9px 20px", background: "rgba(16,185,129,.12)", border: "1px dashed #10b981", color: "#6ee7b7", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>+ Add a leftover from another job / site</button>
              <p style={{ fontSize: 11, color: "#475569", fontFamily: "'Space Mono', monospace", marginTop: 12, lineHeight: 1.6 }}>A leftover only feeds parts of the <b style={{ color: "#94a3b8" }}>same profile &amp; grade</b> — leave grade blank to match parts with no grade. We cut the tightest fit from each leftover first, then buy the minimum new bars for what's left.</p>
            </>
          )}
        </Card>
      )}
    ...
    <>
      ...
      {reusedBarsUsed > 0 && (
        <div style={{ background: "linear-gradient(135deg,#065f46,#047857)", borderRadius: 12, padding: "16px 22px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div><div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: "#d1fae5", letterSpacing: ".1em" }}>♻ LEFTOVERS USED FIRST</div><div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 900, color: "#fff" }}>{reusedBarsUsed} of your leftover bar{reusedBarsUsed > 1 ? "s" : ""} cut before buying new</div></div>
          {reusedSavedKg > 0 && <div style={{ textAlign: "right" }}><div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: "#d1fae5" }}>STEEL NOT PURCHASED</div><div style={{ fontFamily: "'Space Mono', monospace", fontSize: 20, color: "#fff", fontWeight: 700 }}>{fmtKg(reusedSavedKg)}</div></div>}
        </div>
      )}
      {results.groups.map((g, gi) => (
        <div key={gi} style={{ marginBottom: 32 }}>
          ...
          {g.reusedBins && g.reusedBins.some(b => b.cuts.length) && <Card title="♻ Cut from your leftovers — no purchase" style={{ marginBottom: 16 }}><div style={{ fontSize: 12, color: "#6ee7b7", marginBottom: 12, background: "rgba(16,185,129,.08)", border: "1px solid rgba(16,185,129,.25)", borderRadius: 6, padding: "8px 12px" }}>These pieces came out of leftover bars you already owned — they are <b>not</b> in the buy list above.{g.reusedRemainKg ? <> Leftover still remaining after this: <b>{fmtKg(g.reusedRemainKg)}</b>.</> : null}</div>{g.reusedBins.filter(b => b.cuts.length).map((b, i) => <CutBar key={i} bin={b} index={i} />)}</Card>}
          <Card title="Visual Cut Plan">{g.bins.length ? g.bins.map((b, i) => <CutBar key={i} bin={b} index={i} />) : <div style={{ color: "#64748b", fontFamily: "'Space Mono', monospace", fontSize: 12 }}>{g.reusedBins && g.reusedBins.some(x => x.cuts.length) ? "All pieces were cut from your leftovers — nothing new to buy." : "No plan."}</div>}</Card>
        </div>
      ))}
      {(() => {
        ...
        results.groups.forEach(g => {
          g.bins.forEach((b, i) => { if (b.remaining >= REUSE_MIN) rows.push({ profile: g.profile, grade: g.grade, bar: `Bar ${i + 1}`, stock: b.stockLength, len: Math.round(b.remaining), kg: g.kgm ? (b.remaining / 1000) * g.kgm : null }); });
          (g.reusedBins || []).forEach((b, i) => { if (b.cuts.length && b.remaining >= REUSE_MIN) rows.push({ profile: g.profile, grade: g.grade, bar: `Leftover ${i + 1}`, stock: b.stockLength, len: Math.round(b.remaining), kg: g.kgm ? (b.remaining / 1000) * g.kgm : null }); });
        });
        ...
        return (
          <Card title="♻ Reusable Bar Offcuts — leftover steel for the next project" style={{ marginBottom: 16 }}>
            ...
            <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'Space Mono', monospace" }}><thead><tr style={{ background: "rgba(16,185,129,.12)", borderBottom: "1px solid rgba(16,185,129,.3)" }}>{["PROFILE", "GRADE", "FROM BAR", "STOCK LENGTH", "REUSABLE LEFTOVER", "WEIGHT", "STATUS"].map((h, i) => <th key={h} style={{ padding: "10px 12px", textAlign: i > 4 ? "right" : "left", color: "#6ee7b7", fontSize: 11, letterSpacing: 1, fontWeight: 700 }}>{h}</th>)}</tr></thead>
              <tbody>{rows.map((r, i) => <tr key={i} style={{ borderBottom: "1px solid #1a2230" }}><td style={{ padding: "9px 12px", color: "#cbd5e1" }}>{r.profile}</td><td style={{ padding: "9px 12px", color: "#94a3b8" }}>{r.grade || "—"}</td><td style={{ padding: "9px 12px", color: "#94a3b8" }}>{r.bar}</td><td style={{ padding: "9px 12px", color: "#94a3b8" }}>{fmtMm(r.stock)}</td><td style={{ padding: "9px 12px", color: "#6ee7b7", fontWeight: 700 }}>{fmtMm(r.len)}</td><td style={{ padding: "9px 12px", textAlign: "right", color: "#94a3b8" }}>{r.kg != null ? fmtKg(r.kg) : "—"}</td><td style={{ padding: "9px 12px", textAlign: "right", color: "#10B981", fontWeight: 700 }}>✓ keep</td></tr>)}</tbody>
            </table></div>
          </Card>
        );
      })()}
