            ...
            {sec && (() => {
              ...
              return (
                <div>
                  ...
                  <button onClick={closeAll} style={{ marginTop: 14, width: "100%", padding: "11px 16px", borderRadius: 8, border: "1px solid rgba(245,158,11,.5)", background: "rgba(245,158,11,.12)", color: "#fbbf24", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>{tx("useIt")}</button>
                </div>
              );
            })()}
          </div>
  ...
  function autoRead(wb) {
    ...
    wb.SheetNames.forEach(name => {
      ...
      for (let i = 0; i < Math.min(data.length, 30); i++) {
        ...
        const strict = f(/profile|section|member|designation|shape/);
        // Advance Steel and several fabrication tools put the profile under "Description".
        const pi = strict !== -1 ? strict : f(/description|\bsize\b|\bdesc\b/);
        const li = f(/length/);
        if (pi !== -1 && li !== -1) {
          hr = i;
