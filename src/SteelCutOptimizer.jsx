...
function splitPart(W, L, uw, uh, allowRotation) {
  // Split a part too big for the sheet into welded sub-pieces, using the FEWEST
  // pieces / welds. Prefer slicing along ONE direction only (keep pieces as large
  // as possible); fall back to a 2D grid only when one slice can't make pieces fit.
  const sliceOneWay = (a, b) => {
    // keep width = a (must fit a sheet dim), slice length b into sheet-length chunks
    const fitA = (a <= uw) ? uw : (a <= uh ? uh : null);   // which sheet dim holds the full width a
    if (fitA === null) return null;                         // a doesn't fit either sheet dim -> can't 1D slice
    const segMax = (fitA === uw) ? uh : uw;                 // the other sheet dim is the slice length
    const n = Math.ceil(b / segMax);
    const list = [];
    for (let i = 0; i < n; i++) list.push({ w: Math.round(a), h: Math.round(Math.min(segMax, b - i * segMax)) });
    return list;
  };
  const grid = (a, b) => {
  ...
  const candidates = [];
  const s1 = sliceOneWay(W, L); if (s1) candidates.push(s1);
  const s2 = sliceOneWay(L, W); if (allowRotation && s2) candidates.push(s2);
  candidates.push(grid(W, L));
  if (allowRotation) candidates.push(grid(L, W));
  // fewest pieces wins; tie-break on largest minimum piece area (biggest usable chunks)
  candidates.sort((x, y) => x.length - y.length || (Math.min(...y.map(p => p.w * p.h)) - Math.min(...x.map(p => p.w * p.h))));
  return candidates[0];
}
...
function nestPlates(sheetW, sheetH, parts, kerf, margin, allowRotation, reuseMin) {
  ...
  while (queue.length > 0 && guard < 20000) {
    ...
    if (placements.length === 0 && remaining.length > 0) { const p = remaining.shift(); sheets.push({ placements: [{ x: m, y: m, w: Math.min(p.w, usableW), h: Math.min(p.h, usableH), rotated: false, id: p.id, label: p.label, spliced: p.spliced }], offcuts: [] }); }
      ...
      const offcuts = [];
      // Right-side strips of all rows EXCEPT the last (plain rectangles between parts).
      for (let i = 0; i < rows.length - 1; i++) { const r = rows[i]; const stripW = usableW - r.usedW; if (stripW >= reuseMin && r.rowH >= reuseMin) offcuts.push({ shape: "rect", x: m + r.usedW, y: r.y, A: Math.round(stripW), B: Math.round(r.rowH), w: Math.round(stripW), h: Math.round(r.rowH) }); }
      // Main leftover = (last row's right strip) + (everything below the last row).
      // When the last row isn't full width AND there is space below, this is an L-shape.
      const lastRow = rows.length ? rows[rows.length - 1] : null;
      const lastRowH = lastRow ? lastRow.rowH : 0;
      const usedW = lastRow ? lastRow.usedW : 0;
      const lastBottom = lastRow ? lastRow.y + lastRow.rowH : m;
      const bottomH = (m + usableH) - lastBottom;
      const rightW = usableW - usedW;
      const okBottom = bottomH >= reuseMin && usableW >= reuseMin;
      const okRight = rightW >= reuseMin && lastRowH >= reuseMin;
      if (okBottom && rightW >= reuseMin && usedW > 0 && lastRow) {
        // L-shape: overall usableW × (lastRowH + bottomH), notch (parts) at top-left = usedW × lastRowH
        const A = Math.round(usableW), B = Math.round(lastRowH + bottomH), notchW = Math.round(usedW), notchH = Math.round(lastRowH);
        const r1 = { w: rightW, h: lastRowH + bottomH };   // beside the notch (right, full height)
        const r2 = { w: usableW, h: bottomH };             // below the notch (full width)
        const usable = (r1.w * r1.h >= r2.w * r2.h) ? r1 : r2;
        offcuts.push({ shape: "L", x: m, y: lastRow.y, A, B, notchW, notchH, w: Math.round(usable.w), h: Math.round(usable.h) });
      } else if (okBottom) {
        offcuts.push({ shape: "rect", x: m, y: lastBottom, A: Math.round(usableW), B: Math.round(bottomH), w: Math.round(usableW), h: Math.round(bottomH) });
      } else if (okRight && lastRow) {
        offcuts.push({ shape: "rect", x: m + usedW, y: lastRow.y, A: Math.round(rightW), B: Math.round(lastRowH), w: Math.round(rightW), h: Math.round(lastRowH) });
      }
