...
// A profile is a PLATE if it begins with a plate prefix then a number.
// Order of prefixes matters: longer/more-specific first (FLT before FL before F).
const PLATE_PREFIX = /^(PLATE|PLT|PL|FLT|FLAT|FL|FB|P)\s*[-_]?\s*\d/i;

// Hot-rolled / non-plate sections to EXCLUDE (tubes, beams, channels, angles…).
// Checked ONLY after confirming the profile is not a plate.
const HOTROLLED = /^(SHS|RHS|CHS|IPE|IPN|HEA|HEB|HEM|HE|UBP|UB|UC|PFC|UPN|UPE|JIS|HSS|HD|HP|UA|EA|RSA|RSJ|CFSHS|CFRHS|L|W|S|C|MC|BR|RD|ANGLE|CHANNEL|BEAM|TUBE|PIPE|ROUND|SECTION)/i;

function isPlateProfile(profileRaw) {
  return PLATE_PREFIX.test(String(profileRaw || "").trim());
}
function isHotRolled(profileRaw) {
  const p = String(profileRaw || "").trim();
  if (isPlateProfile(p)) return false;       // plates are never hot-rolled
  return HOTROLLED.test(p);
}

// Parse a Tekla profile string → plate dims, or null if not a plate.
// Rule: the two numbers are thickness × width; thickness = the SMALLER number.
function parsePlateProfile(profileRaw) {
  const profile = String(profileRaw || "").trim().toUpperCase();
  if (!profile) return null;
  // plate prefixes: PLATE, PLT, PL, FLT, FLAT, FL, FB, P  e.g. PLT20*270, FLT10*100, PL8*127.3
  const m = profile.match(/^(?:PLATE|PLT|PL|FLT|FLAT|FL|FB|P)\s*[-_]?\s*(\d+(?:\.\d+)?)\s*[*xX×]\s*(\d+(?:\.\d+)?)/);
  if (m) { const a = +m[1], b = +m[2]; return { thickness: Math.min(a, b), width: Math.max(a, b) }; }
  // bare "20*270" or "12x150" with no hot-rolled prefix
  if (!isHotRolled(profile)) {
    const bare = profile.match(/^(\d+(?:\.\d+)?)\s*[*xX×]\s*(\d+(?:\.\d+)?)/);
...
export default function App() {
  ...
  // normalise: strip non-breaking spaces (\xa0 common in Tekla exports) and trim
  const norm = (s) => String(s == null ? "" : s).replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();
  const detectColumn = (headers, aliases) => {
    const H = headers.map(norm);
    for (let i = 0; i < H.length; i++) for (const a of aliases) if (H[i].toLowerCase() === a.toLowerCase()) return headers[i];
    for (let i = 0; i < H.length; i++) for (const a of aliases) if (H[i].toLowerCase().includes(a.toLowerCase())) return headers[i];
    return null;
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        const map = {
          ...
          length: detectColumn(headers, ["length (mm)", "length(mm)", "length", "l (mm)"]),
          width: detectColumn(headers, ["width (mm)", "width(mm)", "width", "breadth"]),
          thickness: detectColumn(headers, ["thickness", "thk", "t (mm)"]),
        };
        const out = extractParts(headers, rows, map);
        if (out.length) { setParts(out); setExcelMsg(`✓ Imported ${out.length} plate part type${out.length > 1 ? "s" : ""}. Hot-rolled sections (SHS, RHS, IPE, UB, UC…) and subtotal rows were ignored automatically.`); setColumnMap(null); }
        else { setColumnMap({ headers, map, rows }); setExcelMsg("⚠ Couldn't auto-detect the columns. Tell me which is Profile, Qty and Length below."); }
  ...
  function extractParts(headers, rows, map) {
    ...
    rows.forEach((row, ri) => {
      ...
      // skip subtotal / summary lines: a bare "Total", "Total for … members", "Grand total", etc.
      if (/^\s*(grand\s+)?total\b/i.test(joined) || /total\s+for|members/i.test(joined)) return;
      // skip a row whose profile cell itself is just "Total"
      ...
      // 1) Profile column (e.g. PLT20*270, FLT10*100). Decide plate vs hot-rolled FIRST.
      const profileCell = pi >= 0 ? cells[pi] : (cells.find(c => isPlateProfile(c) || isHotRolled(c)) || "");
      if (profileCell) {
        if (isHotRolled(profileCell)) return;            // exclude SHS, RHS, IPE, UB, UC, JIS… 
      ...
      const haveProfilePlate = thickness > 0 && width > 0; // dims already known from profile

      // 3) FALLBACK PARSER — only for messy sheets with NO usable profile.
      // Never overwrite dimensions already taken from a recognised plate profile.
      if (!haveProfilePlate && !(length > 0 && width > 0)) {
        const nums = cells.flatMap(c => {
          const pair = c.match(/(\d+(?:\.\d+)?)\s*[*xX×]\s*(\d+(?:\.\d+)?)/);
        ...
        if (nums.length >= 2) {
          ...
          if (!thickness) { const thkCand = sorted.find(n => n >= 3 && n <= 80); thickness = thkCand || 10; }
