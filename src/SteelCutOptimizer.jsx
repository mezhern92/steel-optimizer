...
const LANG_DICT = {
  ...
  // ---- contact footer ----
  ctTitle:        { en: "Stuck? Send me the file.",    ar: "واجهتك مشكلة؟ أرسل لي الملف." },
  ctBody:         { en: "If your material list will not read, or the layout looks wrong, send it to me and I will run it myself and send the cutting plan back. No charge, no signup — I would rather see the file that broke it than lose the job.",
                    ar: "إذا لم تُقرأ قائمة موادك، أو بدا توزيع القص غير منطقي، أرسلها لي وسأشغّلها بنفسي وأعيد لك خطة القص. بدون مقابل وبدون تسجيل — أن أرى الملف الذي فشل أهم عندي من أن أخسر الاستخدام." },
...
function EarlyAccessNotice() {
  ...
  return (
    <div style={{ marginBottom: 22, padding: "14px 18px", borderRadius: 12,
      ...
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, letterSpacing: 2,
        textTransform: "uppercase", color: "#fbbf24", marginBottom: 7 }}>{t("preNoticeTtl")}</div>
      <div style={{ fontSize: 13.5, lineHeight: 1.65, color: "#94a3b8", marginBottom: sent ? 0 : 12 }}>{t("preNoticeBody")}</div>
      {sent ? (
        <div style={{ marginTop: 10, fontSize: 13, color: "#6ee7b7" }}>{t("preNoticeSent")}</div>
      ) : (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <button onClick={send} disabled={busy}
            style={{ padding: "7px 18px", borderRadius: 4, border: "1px solid #d97706",
              ...
              fontSize: 12, opacity: busy ? .6 : 1 }}>{t("preNoticeCta")}</button>
        </div>
      )}
    </div>
  );
...
function PriceCard({ pricing }) {
  ...
  return (
    <Card title={t("priceCardTtl")} style={{ marginTop: 20 }}>
      <div style={{ fontSize: 13, color: "#94a3b8", marginBottom: 14, lineHeight: 1.6 }}>{t("priceCardLead")}</div>
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-end" }}>
        ...
        {pricing.active && <div style={{ paddingBottom: 6, fontFamily: "'Space Mono', monospace", fontSize: 13, color: "#6ee7b7" }}>≈ <b style={{ color: "#34d399", fontSize: 16 }}>{Math.round(pricing.pricePerTon).toLocaleString()}</b> {pricing.currency}/{t("t_ton")}</div>}
      </div>
    </Card>
  );
}
...
function CostBlock({ buyKg, netKg, wasteKg, offcutKg, pricing }) {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ marginTop: 8, fontSize: 11, color: "#64748b", fontFamily: "'Space Mono', monospace", textAlign: "end" }}>{t("costBasedOn", { p: `${Math.round(P).toLocaleString()} ${C}` })}</div>
    </div>
  );
}
...
function PlatesModule({ onBack }) {
  ...
  const parseExcel = useCallback(async file => {
    ...
    try {
      ...
      {
        const wb = XLSX.read(buf, { type: "array" }); const ws = wb.Sheets[wb.SheetNames[0]]; const raw = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
    ...
    <>
      ...
      {inputMode === null && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 20 }}>
            ...
            <Chooser icon="🔍" titleKey="scanTitle" descKey="scanDescP" onClick={() => setInputMode("scan")} />
          </div>
        </div>
      )}
      {inputMode !== null && (
      ...
      {inputMode === "scan" && (
        <Card title={t("scanCardTtl")} style={{ marginTop: 20 }} action={<BackChip onClick={() => setInputMode(null)} labelKey="changeMethod" />}>
          ...
          {excelMsg && <div style={{ marginTop: 12, padding: "8px 12px", borderRadius: 4, background: "rgba(16,185,129,.15)", border: "1px solid rgba(16,185,129,.4)", color: "#6ee7b7", fontSize: 12 }}>{excelMsg}</div>}
        </Card>
      )}
...
async function loadTesseract() {
  ...
  if (!_tesseract) throw new Error("text reader unavailable");
  return _tesseract;
}
...
function ScanImporter({ mode, onUse }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 14, padding: "11px 14px", borderRadius: 6, background: "rgba(245,158,11,.08)", border: "1px solid rgba(245,158,11,.25)", color: "#fcd34d", fontSize: 12, lineHeight: 1.6 }}>{t("scanWarn")}</div>
      ...
      {rows.length === 0 && (
        ...
        >
          <div style={{ fontSize: 42, marginBottom: 12 }}>{busy ? "⏳" : "🔍"}</div>
          {busy ? (
            <div style={{ color: "#fcd34d", fontSize: 14, fontFamily: "'Space Mono', monospace" }}>
              {stage === "ocr" ? t("scanOcr", { pct }) : t("scanReading")}
            </div>
          ) : (
            <>
              <div style={{ color: "#cbd5e1", fontSize: 15, marginBottom: 6 }}>{t("scanDrop")}</div>
              <div style={{ color: "#94a3b8", fontSize: 12 }}>{t("scanAccept")}</div>
              <div style={{ color: "#475569", fontSize: 11, marginTop: 12, lineHeight: 1.7 }}>{t("scanTip")}</div>
            </>
          )}
      ...
      {err && <div style={{ marginTop: 12, padding: "9px 13px", borderRadius: 5, background: "rgba(245,158,11,.14)", border: "1px solid rgba(245,158,11,.4)", color: "#fcd34d", fontSize: 12, lineHeight: 1.6 }}>{err}</div>}

      {rows.length > 0 && (
        <div style={{ marginTop: 4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
            <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, color: "#22c55e" }}>{t("scanFound", { n: rows.length, s: pls(rows.length) })}</span>
            <span style={{ fontSize: 11, color: "#475569" }}>{fileName}</span>
            <button onClick={() => { setRows([]); setErr(""); }} style={{ marginInlineStart: "auto", background: "none", border: "1px solid #2d3748", borderRadius: 4, color: "#94a3b8", cursor: "pointer", fontSize: 11, padding: "5px 12px" }}>{t("scanRedo")}</button>
          </div>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead><tr style={{ borderBottom: "1px solid #2d3748" }}>
                ...
                ).map((h, hi) => <th key={hi} style={{ textAlign: "start", padding: "6px 8px", color: "#64748b", fontWeight: 600, fontSize: 10, letterSpacing: 1, fontFamily: "'Space Mono', monospace" }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #1a2230" }}>
                    ...
                    <td style={{ padding: "5px 4px" }}><button onClick={() => del(i)} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", fontSize: 14 }}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
            <button onClick={add} style={{ padding: "8px 18px", background: "rgba(245,158,11,.15)", border: "1px dashed #d97706", color: "#fbbf24", borderRadius: 4, cursor: "pointer", fontSize: 12 }}>{t("scanAddRow")}</button>
            <button onClick={() => valid.length && onUse(valid)} disabled={!valid.length}
              style={{ padding: "10px 24px", borderRadius: 6, border: "none", cursor: valid.length ? "pointer" : "not-allowed", fontSize: 13, fontWeight: 700, background: valid.length ? "linear-gradient(135deg,#10b981,#059669)" : "#1a2230", color: valid.length ? "#04140d" : "#475569" }}>
              {t("scanUse")}
            </button>
          </div>
        </div>
...
function safeExport(action) {
  ...
  try { action(); }
    ...
    try { alert("The report could not be generated. Please try again.\nتعذّر إنشاء التقرير. حاول مرة أخرى."); } catch { /* no-op */ }
  }
}
...
function SectionLibrary() {
  ...
  const famLabel = (f) => (SL_FAM_LABEL[f] ? SL_FAM_LABEL[f][lang] : f);
  const listRaw = fam ? data[fam] : [];
  const list = q ? listRaw.filter(r => r.name.toLowerCase().includes(q.toLowerCase())) : listRaw;
  ...
  const CHIP = { ...CARD, padding: "12px 14px", cursor: "pointer", textAlign: "start", color: "#cbd5e1", fontSize: 13, lineHeight: 1.5, width: "100%" };
  const areaCm2 = sec ? (sec.kgm / 7850 * 1e4) : 0;
  return (
    <>
      <button onClick={() => setOpen(true)} title={tx("title")}
        style={{ position: "fixed", top: 16, [dir === "rtl" ? "right" : "left"]: 16, zIndex: 10000,
        ...
        <div onClick={closeAll} style={{ position: "fixed", inset: 0, zIndex: 10001, background: "rgba(4,7,12,.78)", backdropFilter: "blur(6px)", display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "6vh 14px 40px" }}>
          <div dir={dir} onClick={e => e.stopPropagation()} style={{ ...CARD, width: "100%", maxWidth: 680, padding: 22, boxShadow: "0 30px 80px -20px rgba(0,0,0,.8)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <div style={{ fontFamily: "'Space Mono', monospace", fontWeight: 700, color: "#fbbf24", fontSize: 15 }}>
            ...
            {!fam && !sec && (<>
              <div style={{ color: "#64748b", fontSize: 12, marginBottom: 12 }}>{tx("pickFam")}</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 10 }}>
                {fams.map(f => (
                  <button key={f} onClick={() => setFam(f)} style={CHIP}>
                    <div style={{ color: "#fbbf24", fontFamily: "'Space Mono', monospace", fontWeight: 700, fontSize: 13 }}>{f}</div>
                    <div style={{ fontSize: 11.5, color: "#94a3b8", marginTop: 2 }}>{famLabel(f).replace(/^[^—]+—\s*/, "")}</div>
                    <div style={{ fontSize: 11, color: "#475569", marginTop: 4 }}>{data[f].length} {tx("sections")}</div>
                  </button>))}
              </div>
            </>)}
            {fam && !sec && (<>
              <input value={q} onChange={e => setQ(e.target.value)} placeholder={tx("search")}
                style={{ width: "100%", boxSizing: "border-box", marginBottom: 12, padding: "10px 14px", borderRadius: 8, background: "#0a0f18", border: "1px solid #1e293b", color: "#e2e8f0", fontSize: 14, outline: "none" }} />
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 8, maxHeight: "52vh", overflowY: "auto", paddingInlineEnd: 4 }}>
                {list.map(r => (
                  <button key={r.name} onClick={() => setSec(r)} style={{ ...CHIP, padding: "10px 12px", display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 12.5, color: "#e2e8f0" }}>{r.name}</span>
                    <span style={{ fontSize: 11.5, color: "#f59e0b", whiteSpace: "nowrap" }}>{r.kgm} kg/m</span>
                  </button>))}
              </div>
            </>)}
            {sec && (() => {
              const dm = SECTION_DIMS[sec.name];
              const asianH = ["JIS-HW","JIS-HM","JIS-HN"].indexOf(sec.type) >= 0;
              const rows = [
                [tx("designation"), sec.name],
                ...(dm ? [
                  [lang === "ar" ? "الارتفاع h" : "Depth h", `${dm[0]} mm`],
                  [lang === "ar" ? "عرض الشفة b" : "Flange width b", `${dm[1]} mm`],
                  [lang === "ar" ? "سماكة الجذع tw" : "Web thickness tw", `${dm[2]} mm`],
                  [lang === "ar" ? "سماكة الشفة tf" : "Flange thickness tf", `${dm[3]} mm`],
                ] : []),
                [tx("family"), famLabel(sec.type)],
                [tx("unitMass"), `${sec.kgm} kg/m`],
                [tx("area"), `${areaCm2.toFixed(1)} cm²`],
                [`${tx("perBar")} — 6 m`, `${(sec.kgm * 6).toFixed(1)} kg`],
                [`${tx("perBar")} — 12 m`, `${(sec.kgm * 12).toFixed(1)} kg`],
                [`${tx("perTonne")} (12 m)`, `${(1000 / (sec.kgm * 12)).toFixed(1)} ${tx("barsOf")} 12 m`],
              ];
              return (
                <div>
                  ...
                  <div style={{ marginTop: 12, fontSize: 11.5, color: "#64748b", lineHeight: 1.7 }}>{tx("note")}{SECTION_DIMS[sec.name] ? <><br />{SL_DIM_SRC[lang]}</> : null}{asianH ? <><br />{lang === "ar" ? "\u0645\u0644\u0627\u062d\u0638\u0629: h\u060c b\u060c tw\u060c tf \u0645\u062a\u0637\u0627\u0628\u0642\u0629 \u0641\u064a JIS G 3192 \u0648 KS D 3502 \u0648 GB/T 11263 \u2014 \u064a\u062e\u062a\u0644\u0641 \u0641\u0642\u0637 \u0646\u0635\u0641 \u0642\u0637\u0631 \u0627\u0644\u062a\u062f\u0648\u064a\u0631\u060c \u0641\u064a\u0646\u062a\u062c \u0641\u0631\u0642 \u0648\u0632\u0646 \u062d\u062a\u0649 2%. \u0627\u0644\u0623\u0648\u0632\u0627\u0646 \u0647\u0646\u0627 \u062a\u062a\u0628\u0639 KS/GB." : "Note: h, b, tw, tf are identical across JIS G 3192, KS D 3502 and GB/T 11263 \u2014 only the root radius differs, giving up to 2% mass difference. Masses here follow KS/GB."}</> : null}</div>
                </div>
              );
            })()}
          </div>
...
──────────────────────────────────────────────────────────────────────────── */
const CONTACT = {
  email: "",              // e.g. "mezher@steeloptimizer.com"
...
function ContactFooter() {
  ...
  const waMsg = encodeURIComponent("Hi, I'm using Steel Optimizer and I need help with my material list.");
  const btn = {
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 20px 72px" }}>
      <div style={{
        ...
        <div style={{ fontSize: 18, fontWeight: 800, color: "#f8fafc", fontFamily: "'Playfair Display', serif", marginBottom: 9 }}>
          {t("ctTitle")}
        </div>
        <div style={{ fontSize: 13.5, color: "#94a3b8", lineHeight: 1.7, marginBottom: 18, maxWidth: 720 }}>
          {t("ctBody")}
        </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {CONTACT.whatsapp && (
            <span style={{ fontSize: 11.5, color: "#64748b", lineHeight: 1.6 }}>{t("ctWhatsHint")}</span>
          )}
        </div>

        {CONTACT.email && (
          <div style={{ marginTop: 14, fontSize: 12, color: "#64748b", fontFamily: "'Space Mono', monospace", wordBreak: "break-all" }}>
            {CONTACT.email}
          </div>
        )}

        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid rgba(148,163,184,.12)", fontSize: 11.5, color: "#64748b", lineHeight: 1.7 }}>
          {t("ctBlocked")}
        </div>
        <div style={{ marginTop: 12, fontSize: 11.5, color: "#475569" }}>{t("ctBy")}</div>
      </div>
    </div>
  );
}
...
function SectionsModule({ onBack }) {
  ...
  const runOpt = list => {
    ...
    alertMe("SOMEONE RAN AN OPTIMISATION (sections)", { profiles: gres.length, rows: list.length });
      ...
      {inputMode === "scan" && (
        <Card title={t("scanCardTtl")} style={{ marginTop: 20 }} action={<BackChip onClick={() => setInputMode(null)} labelKey="changeMethod" />}>
          <ScanImporter mode="sections" onUse={rs => {
            const list = rs.map(r => ({ profile: String(r.profile || "").trim().toUpperCase(), grade: r.grade || "", length: r.length, qty: r.qty, stock: DEFAULT_STOCK }));
            setCutList(list);
            setUploadInfo({ name: t("scanCardTtl"), rows: list.length, pieces: list.reduce((s, r) => s + r.qty, 0), skipped: 0 });
            scrollToOpt();
          }} />
          {uploadInfo && <div style={{ marginTop: 14 }}><Label>{t("lblKerf")}</Label><input type="number" value={kerf} onChange={e => setKerf(+e.target.value)} style={{ ...IN, width: 90 }} /></div>}
        </Card>
      )}
      {(inputMode === "excel" || inputMode === "scan") && uploadInfo && <div ref={optBtnRef} style={{ textAlign: "center", marginTop: 24, scrollMarginTop: 80 }}><button onClick={() => runOpt(cutList)} style={OPT_BTN(false)}>{t("optSections")}</button></div>}
    </>
  );
}
...
function exportPlateExcel(results, material, reuseMin, t, pricing) {
  ...
  if (pricing && pricing.active) {
    ...
    summary.push([], ["STEEL PRICE (per ton)", `${Math.round(P).toLocaleString("en-US")} ${pricing.currency}`, "your entered price"],
      ["TOTAL PURCHASE COST - gross full sheets", M(results.totals.sheetWeight / 1000 * P)],
      ["NET USED VALUE - finished plates only", M(results.totals.partWeight / 1000 * P)],
      ["SCRAP COST - money lost in waste", M(results.totals.wasteWeight / 1000 * P)],
      ["REUSABLE OFFCUT VALUE - recoverable", M(results.totals.offcutWeight / 1000 * P)]);
  }
...
function exportOffcutPDF(results, material, reuseMin) {
  ...
  @media print{button,.btnrow,.hint{display:none}}
  </style></head><body>
  ...
  <div class="btnrow">
  <button class="b-save" onclick="savePDF()">&#8681; Save as PDF &middot; حفظ PDF</button>
  <button class="b-print" onclick="window.print()">&#128424; Print &middot; طباعة</button>
  </div>
  <div class="hint">Both open your browser's print dialog. For a PDF, set <b>Destination</b> to &ldquo;Save as PDF&rdquo;.<br>كلا الزرين يفتحان نافذة الطباعة. لحفظ PDF اختر <b>الوجهة</b> = &ldquo;Save as PDF&rdquo;.</div>
...
function exportPlatePDF(results, material, reuseMin, t, pricing) {
  ...
  if (pricing && pricing.active) {
    const P = pricing.pricePerTon, M = v => `${Math.round(v).toLocaleString("en-US")} ${pricing.currency}`;
    costBox = `<div class="purchase" style="background:#eef6ff;border-color:#2563eb;font-size:15px">Total purchase cost: <b style="font-size:26px;color:#1d4ed8">${M(results.totals.sheetWeight / 1000 * P)}</b> &nbsp;&middot;&nbsp; Net used: <b style="font-size:15px;color:#1a1206">${M(results.totals.partWeight / 1000 * P)}</b> &nbsp;&middot;&nbsp; <span style="color:#b91c1c">Scrap cost: <b>${M(results.totals.wasteWeight / 1000 * P)}</b></span> &nbsp;&middot;&nbsp; <span style="color:#0a7a52">Reusable offcut value: <b>${M(results.totals.offcutWeight / 1000 * P)}</b></span><div style="font-size:11px;color:#777;margin-top:6px">Based on ${Math.round(P).toLocaleString("en-US")} ${pricing.currency}/ton (your entered price)</div></div>`;
  }
...
function exportSectionExcel(results, pricing) {
  ...
  if (pricing && pricing.active) {
    ...
    sum.splice(9, 0,
      ["STEEL PRICE (per ton)", `${Math.round(P).toLocaleString("en-US")} ${pricing.currency} — your entered price`],
      ["TOTAL PURCHASE COST - all raw material incl. waste", M(results.grandKg / 1000 * P)],
      ["NET USED VALUE", M(Math.max(results.grandKg - wasteKg, 0) / 1000 * P)],
      ["SCRAP COST - money in waste", M(wasteKg / 1000 * P)],
      ["REUSABLE OFFCUT VALUE - recoverable", M(offKg / 1000 * P)]);
  }
...
function exportSectionPDF(results, kerf, ow, pricing) {
  ...
  if (pricing && pricing.active) {
    const P = pricing.pricePerTon, M = v => `${Math.round(v).toLocaleString("en-US")} ${pricing.currency}`;
    const offKg = sectionLeftovers(results).reduce((s, r) => s + (r.kg || 0), 0);
    costBox = `<div class="purchase" style="background:#eef6ff;border-color:#2563eb;font-size:15px">Total purchase cost: <b style="font-size:26px;color:#1d4ed8">${M(results.grandKg / 1000 * P)}</b> &nbsp;&middot;&nbsp; Net used: <b style="font-size:15px;color:#1a1206">${M(Math.max(results.grandKg - ow.wasteKg, 0) / 1000 * P)}</b> &nbsp;&middot;&nbsp; <span style="color:#b91c1c">Scrap cost: <b>${M(ow.wasteKg / 1000 * P)}</b></span> &nbsp;&middot;&nbsp; <span style="color:#0a7a52">Reusable offcut value: <b>${M(offKg / 1000 * P)}</b></span><div style="font-size:11px;color:#777;margin-top:6px">Based on ${Math.round(P).toLocaleString("en-US")} ${pricing.currency}/ton (your entered price) &middot; purchase cost covers all raw material bought, including the waste portion.</div></div>`;
  }
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0b0f17", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 480, textAlign: "center", fontFamily: "'Space Mono', monospace" }}>
          ...
          <div style={{ color: "#f8fafc", fontSize: 16, fontWeight: 700, marginBottom: 8 }}>An unexpected error occurred — please reload the page.</div>
          <div style={{ color: "#94a3b8", fontSize: 14, marginBottom: 14 }}>حدث خطأ غير متوقع — أعد تحميل الصفحة.</div>
          <div style={{ color: "#64748b", fontSize: 11, marginBottom: 20, direction: "ltr", wordBreak: "break-word" }}>{String((this.state.err && this.state.err.message) || this.state.err)}</div>
          <button onClick={() => window.location.reload()} style={{ padding: "10px 24px", borderRadius: 8, border: "1px solid #f59e0b", background: "rgba(245,158,11,.15)", color: "#fbbf24", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Reload · إعادة التحميل</button>
        </div>
      </div>
    );
  }
