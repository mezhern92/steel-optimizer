...
/* Find reusable leftover on a sheet by scanning the FREE area (not row boxes),
   so the empty region below short parts (staircase) is captured. Reports the
   biggest empty rectangle, or an L when there's a clean right + bottom region. */
function findOffcuts(placements, m, usableW, usableH, reuseMin) {
  const L = m, R = m + usableW, T = m, B = m + usableH;
  // 1) Honest L: sheet minus the top-left bounding box of all parts (both strips guaranteed empty).
  ...
  const rightW = R - usedRight, bottomH = B - usedBottom;
  const notchW = usedRight - L, notchH = usedBottom - T;

  // 2) Largest single empty rectangle anywhere (candidate top-left corners = part edges & sheet edges).
  const xs = new Set([L]), ys = new Set([T]);
  placements.forEach(p => { xs.add(p.x + p.w); ys.add(p.y); ys.add(p.y + p.h); });
  const xList = [...xs].filter(x => x >= L && x < R - reuseMin + 1).sort((a, b) => a - b);
  const yList = [...ys].filter(y => y >= T && y < B - reuseMin + 1).sort((a, b) => a - b);
  let best = null;
  for (const x0 of xList) for (const y0 of yList) {
    // widen right until blocked, considering only parts overlapping the growing height band
    let wlim = R - x0, hlim = B - y0;
    // height candidates: sheet bottom and each part top below y0's band
    const hCands = [B - y0];
    placements.forEach(p => { if (p.y > y0 && p.x < x0 + (R - x0) && p.x + p.w > x0) hCands.push(p.y - y0); });
    for (const h of hCands) {
      if (h < reuseMin) continue;
      let w = R - x0;
      placements.forEach(p => { if (p.x + p.w > x0 + 0.5 && p.y < y0 + h - 0.5 && p.y + p.h > y0 + 0.5 && p.x >= x0 - 0.5) w = Math.min(w, p.x - x0); });
      if (w >= reuseMin) { const area = w * h; if (!best || area > best.area) best = { x: x0, y: y0, w, h, area }; }
    }
  }

  // Prefer the L when both right and bottom strips are reusable (captures the whole corner region).
